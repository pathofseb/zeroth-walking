...
class ZbotWalkingTask(ksim.PPOTask[ZbotWalkingTaskConfig]):
    ...
    def get_rewards(self, physics_model: ksim.PhysicsModel) -> dict[str, ksim.Reward]:
        robot_config = ROBOT_CONFIGS[self.config.robot]
        rewards = [
            LinearVelocityTrackingReward(scale=10.0),  # train8: Aggressive - high velocity tracking
            AngularVelocityTrackingReward(scale=7.0),  # train8: Aggressive - higher angular velocity
            XYOrientationReward(scale=0.5),  # train8: Aggressive - reduced orientation emphasis
            OrientationPenalty(deadzone_rad=0.2, error_scale=0.15, scale=-2.0),  # train8: Aggressive - reduced penalty
