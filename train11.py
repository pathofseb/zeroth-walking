...
class ZbotWalkingTask(ksim.PPOTask[ZbotWalkingTaskConfig]):
    ...
    def get_rewards(self, physics_model: ksim.PhysicsModel) -> dict[str, ksim.Reward]:
        robot_config = ROBOT_CONFIGS[self.config.robot]
        rewards = [
            LinearVelocityTrackingReward(scale=3.5),  # train11: Conservative - reduced velocity
            AngularVelocityTrackingReward(scale=4.0),  # train11: Conservative - reduced angular velocity
            XYOrientationReward(scale=1.5),  # train11: Conservative - increased orientation
            OrientationPenalty(deadzone_rad=0.1, error_scale=0.3, scale=-7.0),  # train11: Conservative - stricter penalty
            FeetOrientationReward.create(
                ...
                target_rp=(0.0, 0.0),
                error_scale=0.25,
                scale=0.5,  # train11: Conservative - increased feet orientation
            ),
            FeetTooClosePenalty(
                feet_pos_obs_key="feet_position_observation",
                threshold_m=0.12,
                scale=-0.01,  # train11: Conservative - stricter feet positioning
            ),
            FeetTooFarPenalty(
                feet_pos_obs_key="feet_position_observation",
                threshold_m=0.25,
                scale=-3.0,  # train11: Conservative - stricter feet positioning
            ),
            StraightLegPenalty.create_penalty(physics_model, scale=-0.8, scale_by_curriculum=True),  # train11: Conservative
            AnkleKneePenalty.create_penalty(physics_model, scale=-0.04, scale_by_curriculum=True, robot_name=self.config.robot),  # train11: Conservative
            ksim.JointAccelerationPenalty(scale=-0.03, scale_by_curriculum=True),  # train11: Conservative - smoother movements
